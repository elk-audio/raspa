...
void print_usage(char *argv[])
{
    ...
    printf("    -o <output_channel> : Specify the output channel index.\n"
           ...
           "                            Default is %d",
                                        DEFAULT_INPUT_CHANNEL);
    printf("    -f <num_biquad>     : Specify the number of biquad.\n"
           "                            Default is %d.\n",
                                        DEFAULT_BIQUAD_NUM);
    printf("    -d <num_delay>      : Specify the number of delay lines.\n"
           "                            Default is %d.\n",
                                        DEFAULT_DELAY_LINE_NUM);
    printf("    -l <delay_length>   : Specify the memory length in samples of each\n"
           "                            delay line. Default is %d.\n",
                                        DEFAULT_DELAY_LINE_LEN);
    printf("    -s <delay_step>     : Specify the delay step in samples between.\n"
           "                            consecutive delay lines. Default is %d.\n",
...
void biquad_init(void)
{
    if (num_biquad > 0)
    {
        k_mem.biquad = calloc(num_biquad, sizeof(struct biquad_k));
        check_alloc(k_mem.biquad);

        d_mem.biquad = calloc(num_biquad, sizeof(struct biquad_d));
        check_alloc(d_mem.biquad);

        for (int i = 0; i < num_biquad; i++)
        {
            k_mem.biquad->gain = 0.5f / num_biquad;
            k_mem.biquad->a[0] = 1.81531792f;
            k_mem.biquad->a[1] = -0.83098222f;
            k_mem.biquad->b[0] = 0.00391608f;
            k_mem.biquad->b[1] = 0.00783215f;
            k_mem.biquad->b[2] = 0.00391608f;
        }
    }
}
...
void delay_init(void)
{
    if (num_delay > 0)
    {
        k_mem.delay = calloc(num_delay, sizeof(struct delay_k));
        check_alloc(k_mem.delay);

        d_mem.delay = calloc(num_delay, sizeof(struct delay_d));
        ...
        for (int i = 0; i < num_delay; i++)
        {
            ...
            d_mem.delay[i].mem = calloc(delay_line_len, sizeof(float));
...
int main(int argc, char *argv[])
{
    ...
    while ((option = getopt(argc, argv,"hb:i:o:f:d:l:s:")) != -1)
    {
        switch (option)
        {
        case 'h' :
    ...
    printf( "\nRunning with a buffer size of %d, "
            "%d biquads and %d delay lines.\n",
            num_frames, num_biquad, num_delay);
    printf( "Input channel %d selected.\n", input_channel);
    printf( "Output channel %d selected.\n", output_channel);
    if (num_delay > 0)
    {
        printf("Delay line length is %d samples (%lu bytes)\n",
                delay_line_len, delay_line_len * sizeof(float));
        printf("Delay step is %d samples (%lu bytes)\n",
                delay_line_step, delay_line_step * sizeof(float));
    }
    ...
    {
            printf("Error: stress test requires at least 1 input and 1 output channel.\n");
            raspa_close();
            free_mem();
            exit(-1);
    }
